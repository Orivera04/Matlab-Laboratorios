...
% Last Modified by GUIDE v2.5 29-Sep-2016 22:03:26
...
%quiver3([Ejes(1),Ejes(2)],[0,0],[0,0]);

quiver3(Ejes(1),0,0,Ejes(2),0,0,2,'r');
quiver3(Ejes(2),0,0,Ejes(1),0,0,2,'r');

quiver3(0,Ejes(3),0,0,Ejes(4),0,2,'g');
quiver3(0,Ejes(4),0,0,Ejes(3),0,2,'g');

axis tight;
Ejes = axis;
quiver3(0,0,Ejes(6),0,0,Ejes(5),2,'Autoscale','off','color','b');
quiver3(0,0,Ejes(5),0,0,Ejes(6),2,'b');
...
axes(handles.axes40);
[x,y] = meshgrid(A:0.4:B,A:0.4:B);
 [Vx,Vy]=gradient(eval(Fun));
hold on; 
 xlim([A,B]);
 ylim([A,B]);    
 view(2);
 Ejes = axis;
 quiver(Ejes(1),0,Ejes(2),0,2,'r','AlignVertexCenters','on');
 quiver(Ejes(2),0,Ejes(1),0,2,'r','AlignVertexCenters','on');
quiver(0,Ejes(3),0,Ejes(4),2,'g','AlignVertexCenters','on');
quiver(0,Ejes(4),0,Ejes(3),2,'g','AlignVertexCenters','on');
[x,y] = meshgrid(A:0.1:B);
 contour(x,y,eval(Fun));
 [x,y] = meshgrid(A:0.4:B,A:0.4:B);
 quiver(x,y,Vx,Vy,1,'color','b','AlignVertexCenters','on');

 grid on;
 axes(handles.axes41);
 hold on;
  quiver3(x,y,eval(Fun),Vx,Vy,eval(Fun),1,'b','AlignVertexCenters','on');
  [x,y] = meshgrid(A:0.1:B);
...
set(handles.uibuttongroup8,'Visible','on');
set(handles.uibuttongroup25,'Visible','on');
set(handles.uibuttongroup26,'Visible','on')
Opc = get(handles.popupmenu2,'Value');
global Fun;
global A ; global B;
Funa = strrep(Fun,'^','.^');
axes(handles.axes45);
...
z1 = eval(Funa);
surf(x, y, z1);
plano = eval(get(handles.edit148,'String')) * ones(length(y));
z2 = subs(Fun,{'x','y'},{plano,y});
view(3);
Ejes = axis;
[Y,Z] = meshgrid(Ejes(1):1:Ejes(6));
plano = eval(get(handles.edit148,'String')) * ones(length(Z));
 mesh(plano,Y,Z); 
view(3); 
zdiff = z1 - z2;
...
plot3(ones(length(yL))*eval(get(handles.edit148,'String')), yL, zL, '-','Color', 'r', 'LineWidth', 3);
grid on;
axes(handles.axes46);
cla;
hold on;
mesh(plano,Y,Z); 
plot3(ones(length(yL))*eval(get(handles.edit148,'String')), yL, zL, '--','Color', 'b', 'LineWidth', 1);
text(eval(get(handles.edit148,'String'))+0.1,yL(2)+0.1,zL(2),'Corte');
view(3);
grid on;
    case 2
        hold on;
...
surf(x, y, z1);
plano = eval(get(handles.edit148,'String')) * ones(length(y));
z2 = subs(Fun,{'x','y'},{x,plano});
view(3);
Ejes = axis;
[X,Z] = meshgrid(Ejes(1):1:Ejes(6));
plano = eval(get(handles.edit148,'String')) * ones(length(Z));
 mesh(X,plano,Z); 
view(3); 
zdiff = z1 - z2;
...
yL = C(2, 2:end);
zL = interp2(x, y, z1, xL, yL);
plot3(xL, ones(length(xL))*eval(get(handles.edit148,'String')), zL, '-','Color', 'r', 'LineWidth', 3);
grid on;
axes(handles.axes46);
cla;
mesh(X,plano,Z); 
plot3(xL, ones(length(xL))*eval(get(handles.edit148,'String')), zL, '--','Color', 'b', 'LineWidth', 1);
text(xL(2)+0.1,eval(get(handles.edit148,'String'))+0.1,zL(2),'Corte');
 grid on;
end

grid on;
set(handles.edit141,'String',Fun);
S = get(handles.popupmenu2,'String');
L = num2str(get(handles.edit148,'string'))
set(handles.edit142,'String',[S(get(handles.popupmenu2,'Value')) ' = ' L]); 
axes(handles.axes45);
Ejes = axis;
quiver3(Ejes(1),0,0,Ejes(2),0,0,2,'r');
quiver3(Ejes(2),0,0,Ejes(1),0,0,2,'r');
...
function pushbutton32_Callback(hObject, eventdata, handles)
 set(handles.uibuttongroup25,'Visible','on');
  set(handles.uibuttongroup8,'Visible','on');
  set(handles.uibuttongroup26,'Visible','off');
  
%Rectas Tangentes % 
Px = eval(get(handles.edit149,'String'));
Py = eval(get(handles.edit151,'String'));
Pz = eval(get(handles.edit153,'String'));
axes(handles.axes44);
global Fun;
global A ;
global B;
...
switch Opc
 ...
 Funa = strrep(Funa,'.','');
 Dx = diff(sym(Funa),'y');
 Pendiente = subs(Dx,{'x,y'},{Px,Py});
 %Z = 2;
 Tangente = ['z - ' num2str(Pz) '=' char(Pendiente) '*(y - ' num2str(Py) ')'];
 Ecuacion = solve(Tangente,'z');
 y = A-2:0.1:B;
 z = subs(Ecuacion,{'y'},{y});
 Ecuacion = eval(Ecuacion);
 
 hold on;
 plot3(Px*ones(length(y)),y,z,'LineStyle','-','Color','b');
 text(Px,y(floor(length(y)/2)),eval(z(floor(length(z))/2)),'Dx');
 plot3(Px,Py,Pz,'.','markers',16,'color','r');
grid on;
    case 2
 cla;
        [x,y] = meshgrid(A:0.1:B);
 ...
 Funa = strrep(Funa,'.','');
 Dy = diff(sym(Funa),'x');
 Pendiente = subs(Dy,{'x,y'},{Px,Py});
 %Z = 2;
 Tangente = ['z - ' num2str(Pz) '=' char(Pendiente) '*(x - ' num2str(Px) ')'];
 Ecuacion = solve(Tangente,'z');
 x = A-2:0.1:B;
 z = subs(Ecuacion,{'x'},{x});
 Ecuacion = eval(Ecuacion);
 ...
 hold on;
 plot3(x,Py*ones(length(x)),z,'LineStyle','-','Color','r');
 text(x(floor(length(x)/2)),Py,eval(z(floor(length(z))/2)),'Dy');
 plot3(Px,Py,Pz,'.','markers',16,'color','k');
 grid on;
case 3
 cla;
 hold on;
...
axes(handles.axes44);
cla;
 i = eval(get(handles.edit155,'String'));
  j = eval(get(handles.edit156,'String'));
  Funa = strrep(Fun,'.','');
  Gx = char(diff(sym(Funa),'x'));
  Gy = diff(sym(Funa),'y');  
  VecGradiante(1) = subs(Gx,{'x','y'},{Px,Py});
  VecGradiante(2) = subs(char(Gy),{'x','y'},{Px,Py});
  Vector_Unitario = sqrt(i.^2 + j.^2);   
  if(Vector_Unitario ~= 1)
      Vector_A(1) =i / Vector_Unitario;
      Vector_A(2) =j / Vector_Unitario;
  ...
  auxz = Pz;    
  [x,y] = meshgrid(A:0.1:B,A:0.1:B);  
  mesh(x,y,eval(Funa),'edgecolor','k');
  view(3);
   Ejes = axis;
  %quiver3(Auxx,auxy,auxz,eval(VecGradiante(1)),eval(VecGradiante(2)),auxz,0.1,'m','LineWidth',1.5);
  %text(Auxx+0.2,auxy+0.2,auxz,'<Gradiante>');
  Funa = strrep(Funa,'.','');
  z = subs(Funa,{'x','y'},{i,j})
   %quiver3(Px,Py,Pz,i+Px,j+Py,eval(z)+Pz,0.1,'k','LineWidth',1);
  text(0.2,0.2,0,'<Vector unitario>');
  quiver3(0,0,0,i/Vector_Unitario,j/Vector_Unitario,eval(z/Vector_Unitario),0.1,'m','LineWidth',1);
  grid on;   
  Pz = eval(get(handles.edit153,'String'));
  [x,y] = meshgrid(A:0.1:B);
  ...
  Dx = diff(sym(Funa),'y');  
 %Z = 2;
 Tangente = ['y - ' num2str(Py) '=' char(Resultado) '*(x - ' num2str(Px) ')'];
 Ecuacion = solve(Tangente,'y');
 x = A-2:0.1:B;
 %z = subs(Ecuacion,{'y'},{y});
 y = eval(Ecuacion);
Plano = sym([char(VecGradiante(1)) '*(x-' num2str(Px) ')+' char(VecGradiante(2)) '*(y-' num2str(Py) ')+' num2str(0) '=(z-' num2str(Pz) ')']);
Plano_Tangente = solve(Plano,'z');
 hold on;
 plot3(x,y,eval(Plano_Tangente),'LineStyle','-','Color','b');
 plot3(Px,Py,Pz,'.','markers',16,'color','r');
grid on;
